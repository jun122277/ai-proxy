...
更新日: 2026-09-23 / 状態: M0 と M1-01 の下準備は実装済み。M1-02/M1-03 は利用者が担当し、未着手。M2〜M7 は未着手。

学習上重要なタイムアウト、キャンセル/過負荷保護、readiness/drain、メトリクスの設計・実装は利用者が行う。AI はモック、SDK 契約、再現環境、レビューを補助する。[M1 の着手ガイド](exercises/m1-lifecycle.md) に引き継ぎ点を記載する。

Actions の使用量を節約するため、CI は Draft PR の解除時と明示的な手動実行に限定する。通常の検証はローカルで行い、マージ前の最終 commit を push してから Draft を解除して Go チェックを1回実行する。コンテナ/脆弱性検査のリモート実行は `full=true` の明示指定時に限定し、main への push では再実行しない。
